...
{
  DNode *node = NULL;
  if( rout && rout->type == DAO_ROUTINE && rout->routHost.t != DAO_INTERFACE )
...
static DaoType* DaoType_Parse( DaoToken **tokens, int start, int end, int *newpos, DaoNameSpace *ns, DaoClass *cls, DaoRoutine *rout )
{
...
DaoType* DaoParser_ParseTypeName( const char *type, DaoNameSpace *ns, DaoClass *cls, DaoRoutine *rout )
{
  ...
  abtype = DaoType_Parse( tokens->items.pToken, 0, tokens->size-1, & i, ns, cls, rout );
...
int DaoParser_ParseParams( DaoParser *self )
{
  ...
  DaoNameSpace *myNS = self->nameSpace;
  DaoInterface *inter = self->interface;
  ...
  if( ((klass && routine != klass->classRoutine) || inter) 
    ...
    if( strcmp( routine->routName->mbs, hostname ) != 0 ){
      ...
      if( routine->type == DAO_ROUTINE && routine->routHost.t != DAO_INTERFACE ){
      ...
      abstype = klass ? klass->objType : inter->abtype;
      abstype = DaoNameSpace_MakeType( myNS, "self",
          DAO_PAR_NAMED, (DaoBase*)abstype, 0, 0 );
    ...
    */
    ...
    if( tki == DTOK_DOTS ){
      ...
      if( tokens[i]->name == DTOK_COLON ){
        ...
        abstype = DaoType_Parse( tokens, i+1, rb-1, &i, myNS, klass, routine );
  ...
  if( rb+1 < self->tokens->size ){
    ...
    abstype = DaoType_Parse( tokens, rb+2, self->tokens->size-1, &i, myNS, klass, routine );
    if( abstype == NULL || i+1 < self->tokens->size ) goto ErrorParamParsing;
    DString_AppendMBS( pname, "=>" );
    DString_Append( pname, abstype->name );
  }else if( klass && routine->routHost.v.klass == klass && DString_EQ( routine->routName, klass->className ) ){
  ...
  if( routine->routType == NULL ){
    routine->routType = DaoType_New( pname->mbs, DAO_ROUTINE, (DaoBase*) abstype, nested );
  ...
  while( start >= from && start <= to ){
    ...
    */
    ...
    if( tki == DTOK_SEMCO ){
      ...
      abtp = DaoType_Parse( tokens, start+1, to, &start, myNS, self->hostClass, routine );
      if( abtp == NULL ){
    ...
    if( (tki >= DKEY_SUB && tki <= DKEY_FUNCTION && tki2 != DTOK_LB) || tki == DKEY_OPERATOR ){
      ...
      if( (virt | stat) && self->hostClass ==NULL && self->interface ==NULL ){
      ...
      if( start+2 <= to && tokens[start+2]->name == DTOK_COLON2 ){
        ...
        if( rout == NULL ){
          if( self->isInterBody ){
            ...
            rout->routHost.v.inter = self->interface;
            if( rout->routHost.v.inter ) rout->attribs |= stat;
          }else{
            rout = DaoRoutine_New();
            rout->defLine = tokens[start]->line;
            rout->routHost.v.klass = self->hostClass;
            if( rout->routHost.v.klass ) rout->attribs |= virt | stat;
          }
        ...
        if( self->isClassBody ){
          ...
          if( value.t == DAO_ROUTINE ){
            ...
            if( value.v.routine->routHost.v.klass != klass ){
          ...
          DNode *node;
          parser->interface = self->interface;
          rout->routHost.v.inter = self->interface;
          node = DMap_Find( self->interface->methods, self->str );

          if( node ){
            DRoutine *existing = (DRoutine*) node->value.pVoid;
            DRoutine_AddOverLoad( (DRoutine*) existing, (DRoutine*)rout );
            if( value.v.routine->routHost.v.inter != self->interface ){
              /* value.v.routine a dummy routine derived from parent class */
              DArray *atmp = rout->routOverLoad;
              rout->routOverLoad = value.v.routine->routOverLoad;
              value.v.routine->routOverLoad = atmp;
            ...
            DMap_Insert( self->interface->methods, self->str, rout );
            GC_IncRC( rout );
          }
        }else{
      ...
      if( rout->routHost.v.p ) rout->routHost.t = rout->routHost.v.p->type;
      /**/
    ...
    */
    ...
    if( tokens[start]->type == DTOK_IDENTIFIER
      ...
      if( eq >= 0 && colon >= 0 && colon < eq ){
        decl = 1;
        abtp = DaoType_Parse( tokens, colon+1, end, &temp2, myNS, self->hostClass, routine );
        ...
        decl = 1;
        decEnd = colon - 1;
        abtp = DaoType_Parse( tokens, colon+1, end, &end, myNS, self->hostClass, routine );
        if( abtp == NULL ) return 0;
      }else{
        decEnd = end;
...
int DaoParser_PostParsing( DaoParser *self )
{
  ...
  */
  ...
  for(i=0; i<self->routCompilable->size; i++){
    ...
    if( rout->routHost.t == DAO_INTERFACE ) continue;
    ...
    *cst = 0;
    ...
    if( start == left ){
      ...
      if( tki == DTOK_COLON2 && tokens[start+1]->name == DTOK_LB ){
        ...
        if( mm <0 ) return -1;
        abtp = DaoType_Parse( tokens, start+2, mm-1, & newpos, ns, self->hostClass, rout );
...
static int DaoParser_MakeEnclosed( DaoParser *self, int start, int end, int regFix )
{
  ...
  if( rb+1 < lb2 ){
    ...
    abstype = DaoType_Parse( tokens, rb+2, lb2-1, &i, myNS, self->hostClass, routine );
  ...
  DString_AppendMBS( pname, ">" );
  rout->routType = DaoNameSpace_FindType( myNS, pname );
  if( rout->routType == NULL ){
    rout->routType = DaoType_New( pname->mbs, DAO_ROUTINE, (DaoBase*) abstype, nested );
    DaoType_MapNames( routine->routType );
  ...
  if( pos < 0 && tki == DTOK_LB ){
    ...
    if( comma <0 || rb < end ){
      ...
      if( tki == DTOK_LB || tki == DTOK_LSB || tki == DTOK_LCB || tki >DAO_NOKEY1
        ...
        int it, newpos = 0;
        DaoType *abtp = DaoType_Parse( tokens, start+1, rb-1, &newpos, myNS, self->hostClass, routine );
    ...
    if( optype == DAO_OPER_IF ){
      ...
      if( optype == DAO_OPER_FIELD){
        ...
        if( optype == DAO_OPER_AND ){
          ...
          int m = 0;
          DaoClass   *klass = self->hostClass;
          DaoType *type = DaoType_Parse( tokens, start2, end, & m, myNS, klass, routine );

...
static int DaoDecorator_CheckType( DaoType *decoType, DValue *csts, DaoType *ts[], int np )
{
	...
	if( ft->tid != DAO_PAR_NAMED || ft->value.v.type->tid != DAO_ROUTINE ) goto CheckFailed;
	ft = ft->value.v.type;
	...
	for(j=1; j<np; j++){
		...
		if( t->tid == DAO_PAR_NAMED || t->tid == DAO_PAR_DEFAULT ) t = t->value.v.type;
		...
		/*
		   printf( "%p %s %p %s\n", tp->value.v.p, tp->name->mbs, t->value.v.p, t->name->mbs );
		   printf( "%i:  %i\n", k, match );
		 */
		...
		DValue *csts, DaoType *ts[], int np, int code, int def, int *min, int *spec )
{
	...
	*spec = 0;
	...
	for(ifrom=0; ifrom<npar; ifrom++){
		...
		if( abtp->tid == DAO_PAR_NAMED || abtp->tid == DAO_PAR_DEFAULT ) abtp = abtp->value.v.type;
		...
		/*
		   printf( "%p %s %p %s\n", tp->value.v.p, tp->name->mbs, abtp->value.v.p, abtp->name->mbs );
		   printf( "%i:  %i\n", ito, parpass[ito] );
		 */
...
void DRoutine_PassParamTypes( DRoutine *self, DaoType *selftype,
		DValue *csts, DaoType *ts[], int np, int code, DMap *defs )
{
	int npar = np;
	...
	if( (code == DVM_MCALL  || code == DVM_MCALL_TC)
		...
		tps ++;
			...
			&& code != DVM_MCALL && code != DVM_MCALL_TC ){
		/* class DaoClass : CppClass{ cppmethod(); } */
		abtp = self->routType->nested->items.pType[0]->value.v.type;
...
static DaoRoutine* DaoRoutine_GetDecorator( DArray *routTable, DValue *p[], int n )
{
	...
	for(i=0; i<routTable->size; i++){
		...
		for(j=1; j<n; j++){
			...
			match = DaoType_MatchValue( types[k]->value.v.type, pv, defs );
...
int DRoutine_FastPassParams( DRoutine *routine, DValue *obj, DValue *recv[], DValue *p[], DValue *base, int np, int code )
{
	...
	for(ifrom=0; ifrom<npar; ifrom++){
		...
		printf( "%s\n", parType[ip]->value.v.type->name->mbs );
		printf( "%s\n", tp->name->mbs );
#endif
		...
		if( self->parser->parsed == 0 ){
			if( ! DaoParser_ParseRoutine( self->parser ) ){
				...
				if( routp ){ /* XXX */
					...
					retp = routp->value.v.type;
					routp->value.v.type = NULL;
					routp->value.t = 0;
				}
				...
				if( retp ){
					routp->value.v.type = retp;
					routp->value.t = DAO_TYPE;
				}
				DArray_Swap( self->parser->tokens, tokens );
		...
		DString *mbs, int setname )
{
	...
	if( setname && opa == opc && daoBitBoolArithOpers2[code-DVM_MOVE] ){
		...
		if( at->tid == DAO_INTERFACE ){
			node = DMap_Find( at->value.v.inter->methods, mbs );
			rout = (DRoutine*) node->value.pBase;
		}else if( at->tid == DAO_OBJECT ){
			rout = (DRoutine*) DaoClass_FindOperator( at->value.v.klass, mbs->mbs, hostClass );
	...
	if( rout ){
		...
		/* if the operation is used in the overloaded operator,
		   do operation by address */
		if( boolop && rout == (DRoutine*) self ) return inumt;
		if( rout ) ct = rout->routType->value.v.type;
		if( rout == NULL && ct ){
			rout = MatchByParamType( rout2, NULL, NULL, ts, 3, DVM_CALL, &min, &spec );
		}
	}else{
		if( bt->tid == DAO_INTERFACE ){
			node = DMap_Find( bt->value.v.inter->methods, mbs );
			rout = (DRoutine*) node->value.pBase;
		}else if( bt->tid == DAO_OBJECT ){
			rout = (DRoutine*) DaoClass_FindOperator( bt->value.v.klass, mbs->mbs, hostClass );
...
void DaoPrintCallError( DArray *errors, DaoStream *stdio )
{
	...
	for(i=0; i<errors->size; i+=2){
		...
		if( rout->routType->name->mbs[k+1] != '?' ){
			...
			DString_Append( mbs, rout->routType->value.v.type->name );
		}
		DString_AppendMBS( mbs, ";\n" );
...
#define AssertPairNumberType( tp ) \
	...
	if( itp->tid == DAO_PAR_NAMED ) itp = itp->value.v.type; \
	k = itp->tid; if( k == DAO_VALTYPE ) k = itp->value.t; \
...
#define UpdateType( id, tp )  GC_ShiftRC( tp, type[id] ), type[id] = tp
	...
	DaoClass *hostClass = self->tidHost==DAO_OBJECT ? self->routHost->value.v.klass:NULL;
	DaoClass *klass;
	DMap       *tmp;
	DNode      *node;
	...
	for(i=0; i<self->routType->nested->size; i++){
		...
		tt = type[i];
		if( tt ) type[i] = tt->value.v.type; /* name:type, name=type */
	...
	/*
	   ...
	   if( self->routType ) printf( "%p %p\n", hostClass, self->routType->value.v.p );
		...
		case DVM_GETI :
			{
				...
				if( val.t == 0 && val.cst && k && bt->tid == 0 ){ /* a[] */
					...
					if( val.t ){
						...
						if( ct->tid == DAO_PAR_NAMED ) ct = ct->value.v.type;
						if( typed_code ){
							if( k < 0xffff ){
								if( ct->tid >= DAO_INTEGER && ct->tid <= DAO_DOUBLE ){
									vmc->b = k;
				...
				}else if( at->tid == DAO_OBJECT && (rout = (DRoutine*) DaoClass_FindOperator( at->value.v.klass, "[]", hostClass )) ){
					rout2 = rout;
		...
		case DVM_GETMI :
			{
				...
				if( opb == 0 ){
					...
					for(j=1; j<=opb; j++){
						...
						if( tid == DAO_VALTYPE ) tid = type[j]->value.t;
					...
					if( type[opa+1]->tid == DAO_VALTYPE ) check1 = type[opa+1]->value.t;
					if( type[opa+2]->tid == DAO_VALTYPE ) check2 = type[opa+2]->value.t;
					/*
					   printf( "at %s %s\n", at->name->mbs, bt->name->mbs );
					 */
					ts[0] = type[opa+1];
					...
					rout = (DRoutine*) DaoClass_FindOperator( at->value.v.klass, "[]", hostClass );
					if( rout == NULL ) goto WrongContainer;
				}else if( at->tid == DAO_CDATA ){
					rout = (DRoutine*) DaoFindFunction( at->typer, mbs );
					if( rout == NULL ) goto WrongContainer;
				...
				if( rout ){
					...
					rout = MatchByParamType( rout, at, NULL, type+opa+1, opb, DVM_CALL, &min, &spec );
					if( rout == NULL ){
						DRoutine_ShowError( rout2, at, NULL, type+opa+1, opb, DVM_CALL, errors );
						goto InvIndex;
					}
					ct = rout->routType->value.v.type;
				}
		...
		case DVM_GETF :
			{
				...
				if( NoCheckingType( at ) ){
					...
					node = DMap_Find( at->value.v.inter->methods, str );
					if( node ){
		...
		case DVM_SETI :
			{
				...
				case DAO_CLASS :
				case DAO_OBJECT :
					if( (rout=(DRoutine*) DaoClass_FindOperator( ct->value.v.klass, "[]=", hostClass )) == NULL)
						goto InvIndex;
					ts[0] = bt;
					ts[1] = at;
					k = 2;
					...
					rout2 = rout;
					rout = MatchByParamType( rout, ct, NULL, ts, k, DVM_CALL, &min, &spec );
					if( rout == NULL ){
						DRoutine_ShowError( rout2, ct, NULL, ts, k, DVM_CALL, errors );
						goto InvIndex;
					}
					break;
				case DAO_CDATA :
		...
		case DVM_SETMI :
			{
				...
				if( ct->tid == DAO_ARRAY ){
					...
					if( type[opc+1]->tid == DAO_VALTYPE ) check1 = type[opc+1]->value.t;
					if( type[opc+2]->tid == DAO_VALTYPE ) check2 = type[opc+2]->value.t;
					/*
					   printf( "at %s %s\n", at->name->mbs, bt->name->mbs );
					 */
					ts[0] = type[opc+1];
					...
					rout = (DRoutine*) DaoClass_FindOperator( ct->value.v.klass, "[]=", hostClass );
					if( rout == NULL ) goto WrongContainer;
				}else if( ct->tid == DAO_CDATA ){
					rout = (DRoutine*) DaoFindFunction( ct->typer, mbs );
					if( rout == NULL ) goto WrongContainer;
				}else if( ct->tid == DAO_INTERFACE ){
					node = DMap_Find( ct->value.v.inter->methods, mbs );
					if( node == NULL ) goto WrongContainer;
					rout = (DRoutine*)node->value.pBase;
				}else if( ct->typer ){
					rout = (DRoutine*) DaoFindFunction( ct->typer, mbs );
				...
				if( rout ){
					...
					ts[opb] = at;
					/* TODO, self type for class? */
					rout = MatchByParamType( rout, ct, NULL, ts, opb+1, DVM_CALL, &min, &spec );
					if( rout == NULL ){
						DRoutine_ShowError( rout2, ct, NULL, ts, opb+1, DVM_CALL, errors );
						goto InvIndex;
					}
				}
				break;
		...
		case DVM_LOAD :
			{
				...
				if( csts[opa].t == DAO_ROUTINE && type[opc]
					...
					rout = rout2 = (DRoutine*)DaoClass_FindOperator( ct->value.v.klass, "=", hostClass );
					if( rout ){
		...
		case DVM_MCURRY :
			{
				...
				if( at->tid == DAO_ROUTINE ){
					...
					for(j=1; j<=opb; j++){
						...
						if( tt->tid == DAO_PAR_NAMED ) tt = tt->value.v.type;
		...
		case DVM_CALL : case DVM_MCALL :
			{
				...
				rout = NULL;
				if( at->tid == DAO_CLASS ){
					rout = (DRoutine*) at->value.v.klass->classRoutine;
					ct = at->value.v.klass->objType;
				}else if( at->tid == DAO_CDATA ){
				...
				if( rout == NULL ){
					 ...
					 */
					tt = rout->routType;
					if( tt->value.v.type == NULL 
							|| DString_FindChar( tt->name, '@', 0 ) != MAXSIZE
							|| DString_FindChar( tt->name, '?', 0 ) != MAXSIZE ){
					...
					if( (spec && rout->parCount != DAO_MAX_PARAM) || rout->routType->value.v.type ==NULL
						...
						if( rout->type == DAO_ROUTINE && rout != (DRoutine*) self
							...
							DRoutine_PassParamTypes( rout, bt, pp, tp, j, code, defs2 );
							if( DaoRoutine_InferTypes( (DaoRoutine*) rout ) ==0 ) goto InvParam;
							MatchByParamType( rout, bt, pp, tp, j, code, &min, &spec );
						}
					}
					DRoutine_PassParamTypes2( rout, bt, pp, tp, j, code, defs2 );
					if( min >= DAO_MT_SUB ){
				...
				if( at->tid != DAO_CLASS && ! ctchecked && ! (opb & DAO_CALL_COROUT) ) ct = ct->value.v.type;
				...
				if( code == DVM_MCALL && tp[0]->tid == DAO_OBJECT 
						&& (tp[0]->value.v.klass->attribs & DAO_CLS_SYNCHRONOUS) ){
		...
		case DVM_YIELD :
			{
				...
				if( code == DVM_YIELD ){
					...
					if( type[opc]==NULL || type[opc]->tid ==DAO_UDF ) UpdateType( opc, ct );
					AssertTypeMatching( ct, type[opc], defs, 0 );
					AssertTypeMatching( at, tt->value.v.type, defs, 0 );
				}
				break;
			}

...
{
	value = DaoValue_CopyWhenSimple( value );
		...
		case DVM_LOAD :
			{
				...
				if( csts[opa].t == DAO_FUNCTREE && type[opc] && type[opc]->tid ==DAO_ROUTINE){
		...
		case DVM_CALL : case DVM_MCALL :
			{
				...
				}else if( csts[opa].t >= DAO_FUNCTREE && csts[opa].t <= DAO_FUNCTION ){

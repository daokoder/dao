...
DaoValue *dao_none_value = (DaoValue*) & none0;
...
static void DaoSTR_PFind( DaoProcess *proc, DaoValue *p[], int N )
{
	...
	dint start = p[3]->xInteger.value;
	dint end = p[4]->xInteger.value;
...
static void DaoSTR_Match0( DaoProcess *proc, DaoValue *p[], int N, int subm )
{
	DString *self = p[0]->xString.data;
	DString *pt = p[1]->xString.data;
	dint start = p[2+subm]->xInteger.value;
	dint end = p[3+subm]->xInteger.value;
	...
	if( p1 != (size_t)-1 && (subm || capt) ) DString_SubString( self, pt, p1, p2-p1+1 );
...
static void DaoSTR_Capture( DaoProcess *proc, DaoValue *p[], int N )
{
	DString *self = p[0]->xString.data;
	DString *pt = p[1]->xString.data;
	dint start = p[2]->xInteger.value;
	dint end = p[3]->xInteger.value;
	size_t p1=start, p2=end;
	int gid;
	DaoString *subs;
	DaoList *list = DaoProcess_PutList( proc );
...
static void DaoSTR_Change( DaoProcess *proc, DaoValue *p[], int N )
{
	...
	DaoRegex *patt = DaoProcess_MakeRegex( proc, pt, self->wcs ==NULL );
	dint start0 = p[4]->xInteger.value;
	dint end0 = p[5]->xInteger.value;
	dint n, index = p[3]->xInteger.value;
	size_t start = start0, end = end0;
	size_t size = self->size;
	if( start0 <0 ) start0 += self->size;
	if( end0 <0 ) end0 += self->size;
	start = start0;
	end = end0;
...
static size_t DaoList_MakeIndex( DaoList *self, dint index, int one_past_last )
{
	if( index < 0 ) index += self->items.size;
	if( (index < 0) | (index > (self->items.size - 1 + one_past_last)) ) return (size_t)-1;
	return index;
}
static void DaoLIST_Insert( DaoProcess *proc, DaoValue *p[], int N )
{
	...
	size_t pos = DaoList_MakeIndex( self, p[2]->xInteger.value, 1 );
	if( pos == (size_t)-1 ){
		char buffer[100];
...
static void PartialQuickSort( DaoProcess *proc, int entry, int r0, int r1,
		DaoValue **data, size_t first, size_t last, size_t part )
{
...
{
	if( (pos = DaoList_MakeIndex( self, pos, 0 )) == (size_t)-1 ) return NULL;
...
{
	DaoValue **val;
	if( (pos = DaoList_MakeIndex( self, pos, 0 )) == (size_t)-1 ) return 1;
...
int DaoList_Insert( DaoList *self, DaoValue *item, size_t pos )
{
	...
	DaoValue *temp = NULL;
	if( (pos = DaoList_MakeIndex( self, pos, 1 )) == (size_t)-1 ) return 1;
...
void DaoCdata_DeleteData( DaoCdata *self )
{
	...
	if( self->buffer.pVoid == NULL ) DaoCdataBindings_Erase( self->data );
	if( self->attribs & DAO_CDATA_FREE ){
		if( self->buffer.pVoid ){
			dao_free( self->buffer.pVoid );
		}else if( self->data ){
...
{
	if( self->buffer.pVoid == NULL && self->data ) DaoCdataBindings_Erase( self->data );
	self->data = data;
	if( self->buffer.pVoid == NULL && data ) DaoCdataBindings_Insert( data, self );
}
...
void DaoBuffer_Resize( DaoCdata *self, size_t size )
{
	self->size = size;
...
static void DaoBuf_New( DaoProcess *proc, DaoValue *p[], int N )
{
	...
	if( size < 0 || ((proc->vmSpace->options & DAO_EXEC_SAFE) && size > 1000) ){
		DaoProcess_RaiseException( proc, DAO_ERROR, buffer_errors[ size >= 0 ] );
		return;
	}
...
static void DaoBuf_CopyData( DaoProcess *proc, DaoValue *p[], int N )
{
	...
	memcpy( self->buffer.pVoid, cdat->buffer.pVoid, cdat->size );
	self->size = cdat->size;
}
...
static DaoFuncItem cptrMeths[]=
{
	...
	{ DaoBuf_GetString, "getstring( self :cdata, stype :enum<mbs, wcs> = $mbs )=>string" },
	{ DaoBuf_SetString, "setstring( self :cdata, str : string )" },
	{ DaoBuf_GetByte,   "getbyte( self :cdata, index :int, stype :enum<signed,unsigned> = $signed )=>int" },
	{ DaoBuf_GetShort,  "getshort( self :cdata, index :int, stype :enum<signed,unsigned> = $signed )=>int" },
	{ DaoBuf_GetInt,    "getint( self :cdata, index :int, stype :enum<signed,unsigned> = $signed )=>int" },
	{ DaoBuf_GetFloat,  "getfloat( self :cdata, index :int )=>float" },
	{ DaoBuf_GetDouble, "getdouble( self :cdata, index :int )=>double" },
	{ DaoBuf_SetByte,   "setbyte( self :cdata, index :int, value :int, stype :enum<signed,unsigned> = $signed)" },
	{ DaoBuf_SetShort,  "setshort( self :cdata, index :int, value :int, stype :enum<signed,unsigned> = $signed)"},
	{ DaoBuf_SetInt,    "setint( self :cdata, index :int, value :int, stype :enum<signed,unsigned> = $signed)" },

...
static int DaoVmSpace_CompleteModuleName( DaoVmSpace *self, DString *fname, int alib );
...
int DaoVmSpace_RunMain( DaoVmSpace *self, DString *file )
{
	...
	if( file == NULL || file->size ==0 || self->evalCmdline ){
		...
		if( self->evalCmdline ){
			...
			if( DaoProcess_Compile( vmp, ns, self->source, 1 ) ==0 ) return 0;
	...
	res = res && DaoProcess_Compile( vmp, ns, self->source, 1 );
...
static DaoNamespace* 
DaoVmSpace_LoadDaoModuleExt( DaoVmSpace *self, DString *libpath, DArray *args, int run )
{
...
DaoNamespace* DaoVmSpace_LoadModule( DaoVmSpace *self, DString *fname, DArray *reqns, int alib )
{
	DaoNamespace *ns = NULL;
#ifdef DAO_WITH_THREAD
#endif
	...
	printf( "modtype = %i\n", modtype );
#endif
	switch( DaoVmSpace_CompleteModuleName( self, fname, alib ) ){
